...
void Klass::initialize_supers(Klass* k, TRAPS) {
  ...
  if (k == NULL) {
    ...
    if (my_depth < primary_super_limit()) {
      ...
      //整体超过了8个，将部分类放入_secondary_super_cache
      super_check_cell = &_secondary_super_cache;
    }
    //设置_super_check_offset属性
  ...
  if (secondary_supers() == NULL) {
    ...
    Klass* p;
    /**
     * 当前父类大于8个的时候，将多余的父类存储在_secondary_supers数组中
     */
    ...
    //计算_secondary_supers的真实大小，并返回对应的实现接口（直接或间接）
