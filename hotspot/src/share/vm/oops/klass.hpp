...
class Klass : public Metadata {
 ...
 protected:
  ...
  /**
   * 对象布局的综合描述符，不是InstanceKlass或ArrayKlass值为0
   */
  jint        _layout_helper;
  ...
  /**
   * 快速查找supertype的偏移量，超过这个的都会放入_secondary_supers
   */
  ...
  /**
   * 存储当前类父类的指针，默认往上推8个，继承链过长，则放入_secondary_supers
   */
  Klass*      _primary_supers[_primary_super_limit];
  // java/lang/Class instance mirroring this class
  //oopDesc类型，保存的是当前Klass实例表示的java类说对应的Class对象，可以根据此访问类的静态属性
  oop       _java_mirror;
  // Superclass
  //直接父类指针
  Klass*      _super;
  ...
  //当前类的类修饰符，private、final、static/abstract、native
