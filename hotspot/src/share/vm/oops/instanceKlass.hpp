...
class InstanceKlass: public Klass {
 ...
 protected:
  /**
   * 实例的构造放阿飞
   * @param vtable_len 虚函数个数
   * @param itable_len 接口函数个数
   * @param static_field_size 静态字段大小
   * @param nonstatic_oop_map_size 非静态
   * @param rt
   * @param access_flags
   * @param is_anonymous
   */
 ...
 protected:
  ...
  /**
   * 非静态字段需要占用的内存空间，以字节为单位，申请空间的时候，会参考此属性
   */
 ...
 //虚方法表的个数
   ...
   * 类初始化的状态，包括
   * - allocated 已分配
   * - laoded  读取Class文件信息并加载到内存中
   * - linked 链接和校验
   * - being_initialized 正在初始化
   * - fully_initialzed 已初始化完成
 ...
 public:
  ...
  // Sizing (in words)
  /**
   * HeapWordSize 在64位系统下是8
   * 使用align_object_offset内存对齐
   * @return 占用的空间大小
   */
  static int header_size()            { return align_object_offset(sizeof(InstanceKlass)/HeapWordSize); }
 /**
  * 计算InstanceKlass实例的大小
  * @param vtable_length 虚表大小
  * @param itable_length 接口表大小
  * @param nonstatic_oop_map_size
  * @param is_interface
  * @param is_anonymous
  * @return
  */
